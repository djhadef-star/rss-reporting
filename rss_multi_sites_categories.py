...
# Normalisation préalable des mots-clés
NORM_CATEGORIES = {
...
def categorize(title, description):
    ...
    for category, keywords in NORM_CATEGORIES.items():
        for kw in keywords:
            # Utilisation de regex \b pour vérifier les mots entiers
...
if not df.empty:
    ...
    # Formatage de la date uniquement au moment de l'export
    df["date"] = df["date"].dt.strftime("%d/%m/%Y %H:%M")

# Création du dossier si nécessaire et export
os.makedirs("output", exist_ok=True)
df.to_csv("output/rss_history.csv", index=False, encoding="utf-8-sig")import os
import re
import unicodedata
from datetime import datetime
import feedparser
...
# Création du dossier si nécessaire et export
os.makedirs("output", exist_ok=True)
df.to_csv("output/rss_history.csv", index=False, encoding="utf-8-sig")